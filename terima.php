...
// Proses terima peminjaman jika ID terdefinisi
if ($id) {
    // Lakukan proses terima peminjaman
    // Contoh query update status
    $sql = "UPDATE riwayat_peminjaman SET Status='Diterima' WHERE id=$id";

    if ($conn->query($sql) === TRUE) {
        // Panggil fungsi untuk mengirim notifikasi ke mahasiswa (contoh fungsi)
        sendNotification($id, 'Diterima');

        // Redirect ke halaman konfirmasi dengan pesan
        header("Location: konfirmasi.php?id=$id&action=terima");
        exit();
    } else {
